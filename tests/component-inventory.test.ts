...
describe('Component Inventory', () => {
  ...
  it('should have all .tsx components in inventory', async () => {
    ...
    // Normalize paths to use forward slashes (inventory uses forward slashes)
    ...
    for (const file of componentFiles) {
      ...
      const isInInventory = inventoryPaths.includes(normalizedFile);
      expect(
        isInInventory,
        `Component not in inventory: ${normalizedFile}\nAdd it to tests/component-inventory.json`
      ).toBe(true);
    }
  });
});
